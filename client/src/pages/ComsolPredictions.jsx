...
const ComsolPredictions = () => {
  ...
  useEffect(() => {
    ...
    if (!predictions.some((item) => item.module === selectedModule)) {
      setSelectedModule(predictions[0].module);
    }

    setSelectedModules(predictions.slice(0, 4).map((item) => item.module));
  }, [predictions]);
  ...
  const fetchPredictions = async () => {
    try {
      ...
      const [predictionResponse, trendResponse] = await Promise.all([
        axios.get(
          `http://localhost:5001/api/comsol/predictions/predictions?period=${selectedPeriod}`,
        ),
        axios.get(
          `http://localhost:5001/api/comsol/predictions/trend?period=${selectedPeriod}`,
        ),
      ]);
      ...
      if (predictionResponse.data.status === "insufficient_data") {
        ...
        setMetrics(predictionResponse.data.metrics);
...
export default ComsolPredictions;
