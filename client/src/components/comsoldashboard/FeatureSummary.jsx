...
const FeatureSummary = ({ selectedPeriod }) => {
  ...
  const fetchSummary = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5001/api/comsol-dashboard/feature-summary?period=${selectedPeriod}`,
      );
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="h-[320px] overflow-y-auto overflow-x-hidden rounded-xl border border-slate-200 dark:border-slate-700">
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-white dark:bg-slate-900 z-10 shadow-sm">
            <tr className="border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">
              <th className="text-left py-3 pl-3 font-semibold">Feature</th>

              <th className="text-right py-3 pr-2 font-semibold">OUT</th>

              <th className="text-right py-3 pr-2 font-semibold">IN</th>

              <th className="text-right py-3 pr-2 font-semibold">Denied</th>

              <th className="text-right py-3 pr-2 font-semibold">Queued</th>

              <th className="text-right py-3 pr-2 font-semibold">Dequeued</th>

              <th className="text-right py-3 pr-2 font-semibold">Peak</th>

              <th className="text-right py-3 pr-3 font-semibold">Users</th>
            </tr>
          </thead>
          ...
          <tbody>
            {data.map((row) => (
              ...
              >
                ...
                <td className="text-right pr-2 text-red-600 dark:text-red-400 font-semibold">
