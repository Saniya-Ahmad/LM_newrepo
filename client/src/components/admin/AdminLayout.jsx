...
const AdminLayout = () => {
  ...
  const linkClasses = (path, isMobile = false) => {
    const base = "block w-full text-left px-4 py-3 rounded-2xl text-sm font-medium transition";
    if (isActive(path)) {
...
>      <header className={`shadow-xl transition-colors duration-300 ${
    isDark
      ...
      : "bg-blue-600"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className={`h-12 w-12 rounded-full overflow-hidden flex items-center justify-center ${isDark ? "bg-white/10" : "bg-slate-100"}`}>
            ...
            <div>
              ...
              <p className={`text-sm ${isDark ? "text-sky-100" : "text-slate-500"}`}>Admin Workspace</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
            ...
            <div className={`rounded-full px-4 py-2 text-sm font-medium ${isDark ? "bg-white/10 text-white" : "bg-slate-100 text-slate-900"}`}>
        ...
        <div className="md:hidden border-b border-slate-800 bg-slate-950 px-4 py-4 text-slate-100">
          <div className="space-y-4">
            <div className="rounded-3xl border border-slate-800 bg-slate-950 p-3">
              ...
              >
                <span>MICROSOFT</span>
                <span className="text-xs text-slate-400">{microsoftOpen ? "Hide" : "Show"}</span>
              </button>
              {microsoftOpen && (
                <div className="mt-3 space-y-2">
                  {microsoftItems.map((item) => (
                    <Link key={item.to} to={item.to} className={linkClasses(item.to, true)}>
                      {item.label}
                    </Link>
                  ))}
                </div>
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-950 p-3">
              ...
              >
                <span>MATLAB</span>
                <span className="text-xs text-slate-400">{matlabOpen ? "Hide" : "Show"}</span>
              </button>
              {matlabOpen && (
                <div className="mt-3 space-y-2">
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-950 p-3">
              ...
              >
                <span>MSC</span>
                <span className="text-xs text-slate-400">{mscOpen ? "Hide" : "Show"}</span>
              </button>
              {mscOpen && (
                <div className="mt-3 space-y-2">
                  {mscItems.map((item) => (
      ...
      <div className="flex min-h-[calc(100vh-5rem)]">
        <aside className={`hidden md:flex flex-col w-80 shrink-0 border-r px-4 py-6 ${isDark ? "border-slate-900 bg-slate-950" : "border-slate-200 bg-white"}`}>
          <div className={`flex items-center gap-3 pb-6 border-b mb-6 ${isDark ? "border-slate-800" : "border-slate-200"}`}>
            <div className="h-12 w-12 rounded-full overflow-hidden bg-slate-900 text-white flex items-center justify-center font-bold">
              A
            </div>
            <div>
              <p className={`text-base font-semibold tracking-wide ${isDark ? "text-white" : "text-slate-900"}`}>
                License Analytics
              </p>
              <p className={`text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>Admin workspace</p>
            </div>
          </div>

          <div className="space-y-4">
            <div className={`rounded-3xl border p-3 ${isDark ? "border-slate-900 bg-slate-900" : "border-slate-200 bg-white"}`}>
              <button
                type="button"
                onClick={() => setMicrosoftOpen((prev) => !prev)}
    ...
    isDark ? "bg-slate-950" : "bg-slate-100"
          ...
          <div  className={`rounded-3xl p-6 min-h-[calc(100vh-5rem)] shadow-lg transition-colors duration-300
  ${
    isDark
